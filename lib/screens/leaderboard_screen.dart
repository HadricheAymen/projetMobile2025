  ...
  Future<void> _fetchCategories() async {
    try {
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          categories = [{'id': 0, 'name': 'All'}, ...data['trivia_categories']];
        });
      } else {
  ...
  void _applyFilters() {
    setState(() {
      filteredResults = allResults.where((result) {
        ...
        if (selectedDifficulty != 'All' && result.difficulty != selectedDifficulty) {
          return false;
        }
        
        // Apply question count filter
        if (selectedQuestionCount != 0 && result.totalQuestions != selectedQuestionCount) {
          return false;
        }
                  ...
                  DropdownButton<int>(
                    ...
                    items: questionCounts.map((count) {
                      return DropdownMenuItem<int>(
                        value: count,
                        child: Text(count == 0 ? 'All' : count.toString()),
                      );
                    }).toList(),
                    onChanged: (value) {
                      setState(() {
